...
import heroImage from "@/assets/technologies/02-ThreatIntelligence.jpg";

const ThreatIntelligence = () => {
  const { t } = useTranslation('threatIntelligence');

  return (
    <div className="min-h-screen bg-white dark:bg-neutral-950">
      {/* Breadcrumb */}
      <nav className="bg-muted py-4 dark:bg-neutral-900">
        <div className="container-width px-6">
          <div className="flex items-center space-x-2 text-sm text-muted-foreground">
            <Link to="/" className="hover:text-primary">{t('common.home', 'Home')}</Link>
            <span>›</span>
            <span>{t('common.technologies', 'Technologies')}</span>
            <span>›</span>
      ...
      {/* Overview Section */}
      <section className="section-padding bg-muted dark:bg-neutral-900">
        <div className="container-width">
        ...
        <div className="container-width">
          ...
          <div className="grid md:grid-cols-2 gap-6 max-w-5xl mx-auto">
            {(t('services.items', { returnObjects: true }) as any[]).map((service: any, index: number) => (
              <div key={index} className="flex items-start gap-3 p-4 bg-muted dark:bg-neutral-900 rounded-lg ring-1 ring-black/5 dark:ring-white/10">
